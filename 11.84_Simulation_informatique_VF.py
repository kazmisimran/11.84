...
def define_all_variables():
    ...
    try:
        ...
        hauteur_masse_deplacee = float(input("Hauteur de la masse déplacée [m] : "))
...
def print_values():
    ...
    try:
        return("Masse de la barge = " + str(masse_barge) + " kg" + "\n"
               ...
               "Hauteur de la masse déplacée = " + str(hauteur_masse_deplacee) + " m" + "\n"
               "Distance de déplacement : " + str(distance_deplacement) + " m")
    except NameError:
        print("Il manque une (ou plusieurs) valeur(s)")

def do_masse_totale(masse_barge,masse_grue,masse_deplacee):
    """
    pré: toutes les masses en kg
    post: masse totale en kg
    """
    global masse_totale
    masse_totale = masse_barge + masse_grue + masse_deplacee   # Calcule de la somme des différentes masses qui composent le système.
    return masse_totale
...
def do_hauteur_centre_gravité_total(hauteur_barge,masse_barge,hauteur_grue,masse_grue,hauteur_masse_deplacee,masse_totale,ligne_flottaison,masse_deplacee):
    """
    ...
    c = ((hauteur_barge + (hauteur_masse_deplacee/2)) * masse_deplacee) 
...
def do_inclinaison(ligne_flottaison,hauteur_barge,largeur_barge,longueur_barge,masse_totale,masse_barge,masse_deplacee,distance_deplacement,hauteur_centre_gravité_total):
    ...
    while theta < (pi/2):
        ...
        H_t = (largeur_barge / cos(theta))   # Hypoténuse du triangle.
        ...
        OG_r = ((h_t + h_r) / 2)   # Distance entre l'origine, milieu de l'hypoténuse du triangle, et le centre de gravité du rectangle.
        ...
        x_a = h_t * ((-sin(theta)) / 2)
        z_a = h_t * ((-cos(theta)) / 2)
        # Coordonnées du sommet du triangle à l'intersection de l'hypoténuse et de la hauteur du triangle.
        x_b = H_t / 2
        z_b = 0
...
def execute_all():
    ...
    '''
    return("Masse totale : " + str(do_masse_totale(masse_barge,masse_grue,masse_deplacee)) + " kg" + "\n"
           ...
           "Centre de gravité total : " + str(do_hauteur_centre_gravité_total(hauteur_barge,masse_barge,hauteur_grue,hauteur_grue,hauteur_masse_deplacee,masse_totale,ligne_flottaison,masse_deplacee)) + " m" + "\n"
           ...
           "Masse supplementaire pour optimiser l'angle d'inclinaison max : " + str(optimisation_angle_max(longueur_barge,largeur_barge,hauteur_barge,masse_totale)) + " kg" + "\n")
    ...
    while True :
        ...
        try :
            ...
            if command_asked == "help" :
                ...
                if sub_command_asked == "hauteur de la masse déplacée" :    
                    hauteur_masses_restantes = float(input("Nouvelle hauteur de la masse déplacée: "))
